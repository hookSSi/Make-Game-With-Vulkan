...
private:
	...
	void Cleanup()
	{
		...
		{
			DestroyDebugUtilsMessengerEXT(appInstance, debugMessenger, nullptr);
		}
	...
	void CreateInstance()
	{
		...
		CheckGlfwRequiredExtensionsValid();

		if (vkCreateInstance(&createInfo, nullptr, &appInstance) != VK_SUCCESS)
		{
	...
	void SetupDebugMessenger()
	{
		...
		if (CreateDebugUtilsMessengerEXT(appInstance, &createInfo, nullptr, &debugMessenger) != VK_SUCCESS)
		{
	...
	void CreateSurface()
	{
		if (glfwCreateWindowSurface(appInstance, window, nullptr, &surface) != VK_SUCCESS)
		{
	...
	// 이런식으로 GPU에 점수를 매겨서 선택할 수 있는 방법도 있다.
	uint32_t RateDeviceSuitablility(VkPhysicalDevice device)
	{
		VkPhysicalDeviceProperties deviceProperties;
		VkPhysicalDeviceFeatures deviceFeatures;
		vkGetPhysicalDeviceProperties(device, &deviceProperties);
	...
	void CreateLogicalDevice()
	{
		...
		if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &appDevice) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to create logical device!");
		}

		vkGetDeviceQueue(appDevice, indices.graphicsFamily.value(), 0, &graphicsQueue);
		vkGetDeviceQueue(appDevice, indices.presentFamily.value(), 0, &presentQueue);
	}
	...
	VkSurfaceFormatKHR ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &availableFormats)
	{
		...
		// 보통 어떤 포멧이 가장 좋은 지 점수를 매기지만
		// 그냥 젤 처음 인덱스를 반환해도 괜찮다.
	...
	/// keep resulution always equal to the resolution of the window that we're drawing to
	...
	/* Checking validation in application */
	// I wrote it myself...
	void CheckGlfwRequiredExtensionsValid()
	{
